            ...
            if (ns->cntr == 8) {
              ...
              if (ns->cmd == NETIO_CMD_C2S) {
                ...
                ns->state = NETIO_STATE_SEND_DATA;
				 //开始时的时间
                ns->time_stamp = OSTimeGet(&os_err);  //获取时间
...
static err_t netio_sent(void *arg, struct tcp_pcb *pcb, u16_t len)
{
  ...
  if (ns->cntr >= ns->data_len && ns->state == NETIO_STATE_SEND_DATA) {
    ...
    if (OSTimeGet(&os_err) - ns->time_stamp > 600) {
      ns->buf_ptr[0] = 1;
      ns->state = NETIO_STATE_SEND_DATA_LAST;
    } else {
      ns->buf_ptr[0] = 0;
...
//接收数据回调函数
static err_t netio_socketrecv(void *arg, struct tcp_pcb *pcb, struct pbuf *p, err_t err)
{
  struct netio_state *ns = arg;
  u8_t * data_ptr;
  u32_t data_cntr;
...
//发送数据回调函数
static err_t netio_socketsent(void *arg, struct tcp_pcb *pcb, u16_t len)
{
  struct netio_state *ns = arg;
  err_t err = ERR_OK;
	OS_ERR os_err;
