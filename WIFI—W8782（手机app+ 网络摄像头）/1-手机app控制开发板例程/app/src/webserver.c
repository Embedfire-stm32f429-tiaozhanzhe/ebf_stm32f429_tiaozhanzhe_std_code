...
const char http_jpeg_head[] = "HTTP/1.1 200 OK\r\n\
Server: WSUM-WebServer-v1.0\r\n\
Connection: Keep-Alive\r\n\
...
<center> \r\n\
<table width=\"400\" height=\"400\" border=\"0\" bordercolor=\"#000000\" bgcolor=\"#eeeeee\">\r\n";
...
const char str_eth_ip_addr_page[] = "eth_ip_addr.html";
...
//const char str_audio_ctl_page[] = "audio.html";	
const char str_pic_ctl_page[] = "picture.html";	
...
const char str_test_pic[]  = "test.jpeg";

#define WEB_BUFF_SIZE 1024
...
<input name=\"ap_ssid\" type=\"text\" id=\"ap_ssid\" maxlength=\"32\"/>\r\n\
</span></div></td>\r\n\
<td width=\"24%\">&nbsp;</td>\r\n\
</tr>\r\n";
...
<input name=\"ap_key\" type=\"text\" id=\"ap_key\" maxlength=\"32\"/>\r\n\
</span></div></td>\r\n\
<td>&nbsp;</td>\r\n\
</tr>\r\n";
...
<label></label>\r\n\
<div align=\"left\">\r\n\
<select name=\"ap_channel\" size=\"1\" id=\"ap_channel\">\r\n";
...
const char local_text7_2[]  = "<select name=\"ap_security\" size=\"1\" id=\"ap_security\">\r\n\
...
const char local_text7_4[]  ="<tr>\r\n\
    <td height=\"43\" colspan=\"3\">&nbsp;</td>\r\n\
  </tr>\r\n\
  <tr>\r\n\
    <td height=\"43\" colspan=\"3\"><div align=\"center\"><span class=\"STYLE18\">STA配置</span></div>\r\n\
        <hr /></td>\r\n\
  </tr>\r\n";

const char local_text7_5[]  ="<tr>\r\n\
    <td height=\"37\"><div align=\"right\"><span class=\"STYLE16\">名称 :</span></div></td>\r\n\
    <td><div align=\"left\"><span class=\"STYLE16\">\r\n\
        <input name=\"sta_ssid\" type=\"text\" id=\"sta_ssid\" maxlength=\"32\"/>\r\n\
    </span></div></td>\r\n\
    <td>&nbsp;</td>\r\n\
  </tr>\r\n";

const char local_text7_6[]  ="<tr>\r\n\
    <td height=\"29\"><div align=\"right\"><span class=\"STYLE16\">密码 :</span></div></td>\r\n\
    <td><div align=\"left\"><span class=\"STYLE16\">\r\n\
        <input name=\"sta_key\" type=\"text\" id=\"sta_key\" maxlength=\"32\"/>\r\n\
    </span></div></td>\r\n\
    <td>&nbsp;</td>\r\n\
  </tr>\r\n";
...
const char local_text_apssid[]  = "<input name=\"ap_ssid\" type=\"text\" id=\"ap_ssid\" maxlength=\"32\"/>";
const char local_text_apkey[]  = "<input name=\"ap_key\" type=\"text\" id=\"ap_key\" maxlength=\"32\"/>";

const char local_text_stassid[]  = "<input name=\"sta_ssid\" type=\"text\" id=\"sta_ssid\" maxlength=\"32\"/>";
const char local_text_stakey[]  = "<input name=\"sta_key\" type=\"text\" id=\"sta_key\" maxlength=\"32\"/>";
...
int wifi_link_page_assemble(UCHAR s, CHAR _sub_step)
{
	...
	switch(step)
	{
		...
		case 4:
		{
			...
			sprintf(tmp, " value=\"%s\" ", web_cfg.ap.essid);
		...
		case 5:
			...
			sprintf(tmp, " value=\"%s\" ", web_cfg.ap.key);
		...
		case 7:
			...
			sprintf((char*)tmp, "option value=\"%d\"", web_cfg.ap.channel);
		...
		case 10:
			...
			sprintf((char*)tmp, "option value=\"%d\"", web_cfg.ap.auth);
		...
		case 12:
			step = 13;
			//assert(SIZEOF_STR(local_text7_3) < WEB_BUFF_MAX_VALID);
			memcpy(web_buff, local_text7_4, SIZEOF_STR(local_text7_4));
			web_buff[SIZEOF_STR(local_text7_4)] = 0;
		...
		case 13:
			step = 14;
			//assert(SIZEOF_STR(local_text7_3) < WEB_BUFF_MAX_VALID);
			memcpy(web_buff, local_text7_5, SIZEOF_STR(local_text7_5));
			web_buff[SIZEOF_STR(local_text7_5)] = 0;
			sprintf(tmp, " value=\"%s\" ", web_cfg.sta.essid);
		...
		case 14:
			step = 15;
			//assert(SIZEOF_STR(local_text7_3) < WEB_BUFF_MAX_VALID);
			memcpy(web_buff, local_text7_6, SIZEOF_STR(local_text7_6));
			web_buff[SIZEOF_STR(local_text7_6)] = 0;
			sprintf(tmp, " value=\"%s\" ", web_cfg.sta.key);
...
int _ip_addr_page_assemble(UCHAR s, CHAR _sub_step, struct netif *p_netif, int auto_get)
{
	static uint8_t step = 0;
	char tmp[64];
	uint32_t dns_server;
...
{
	if(p_netif)
		return _ip_addr_page_assemble(s, _sub_step, p_netif, web_cfg.ip.auto_get);
	else
		return 0;
}

int ip_eth_addr_page_assemble(UCHAR s, CHAR _sub_step)
{
	if(p_eth_netif)
		return _ip_addr_page_assemble(s, _sub_step, p_eth_netif, web_cfg.eth_ip.auto_get);
	else
		return 0;
}
...
const char remote_text1[]  = "<tr>\r\n\
    <td height=\"23\"><div align=\"right\"><span class=\"STYLE16\">采样频率: </span></div>\r\n\
        <span class=\"STYLE16\">\r\n\
        <label></label>\r\n\
        </span>\r\n\
        <div align=\"right\"></div></td>\r\n\
    <td><div align=\"left\"><span class=\"STYLE16\">\r\n\
        <input name=\"audio_fre\" type=\"text\" id=\"audio_fre\" maxlength=\"6\" />\r\n\
    </span>(0-1000)</div></td>\r\n\
  </tr>\r\n";

const char remote_text2[]  = "<tr>\r\n\
    <td width=\"32%\" height=\"38\"><div align=\"right\"><span class=\"STYLE16\">压缩质量: </span></div>\r\n\
        <span class=\"STYLE16\">\r\n\
        <label></label>\r\n\
        </span>\r\n\
        <div align=\"right\"></div></td>\r\n\
    <td width=\"68%\"><div align=\"left\"><span class=\"STYLE16\">\r\n\
      <input name=\"audio_qua\" type=\"text\" id=\"audio_qua\" maxlength=\"6\" />\r\n\
    </span>ms</div></td>\r\n\
  </tr>\r\n";
  


const char audio_fre_text[]  = "<input name=\"audio_fre\" type=\"text\" id=\"audio_fre\" maxlength=\"6\" />";
const char audio_qua_text[]  = "<input name=\"audio_qua\" type=\"text\" id=\"audio_qua\" maxlength=\"6\" />";


int audio_page_assemble(UCHAR s, CHAR _sub_step)
{
	static uint8_t step = 0;
	char tmp[64];
	//unsigned char ip_addr[4];
	...
	switch(step)
	{
		...
		case 3:
			...
			sprintf(tmp, " value=\"%u\" ", web_cfg.audio.audio_fre);
			web_body_add_input_str(web_buff, 
				audio_fre_text, 
				tmp);
  			ret = web_data_send(s, web_buff, strlen(web_buff));
		...
		case 4:
			step = 5;
			assert(SIZEOF_STR(remote_text2) < WEB_BUFF_MAX_VALID);
			memcpy(web_buff, remote_text2, SIZEOF_STR(remote_text2));
			web_buff[SIZEOF_STR(remote_text2)] = 0;
			sprintf(tmp, " value=\"%u\" ", web_cfg.audio.audio_qua);
			web_body_add_input_str(web_buff, 
				audio_qua_text,
				tmp);
  			ret = web_data_send(s, web_buff, strlen(web_buff));
...
const char pic_show_text[]  = "<tr>\r\n\
    <td><div align=\"center\"><img src=\"test.jpeg\" width=\"320\" height=\"240\" /></div></td>\r\n\
    </tr>\r\n";
    
int pic_page_assemble(UCHAR s, CHAR _sub_step)
{
	static uint8_t step = 0;
	//char tmp[64];
	//unsigned char ip_addr[4];
	...
	switch(step)
	{
		...
		case 2:
			step = 5;
			make_sub_page_title("图像预览");
  			ret = web_data_send(s, web_buff, strlen(web_buff));
			return ret;
		case 5:
			step = 6;
			memcpy(web_buff, pic_show_text, SIZEOF_STR(pic_show_text));
			web_buff[SIZEOF_STR(pic_show_text)] = 0;
  			ret = web_data_send(s, web_buff, strlen(web_buff));
			return ret;	
		case 6:
			step = 7;
...
int uart_page_assemble(UCHAR s, CHAR _sub_step)
{
	...
	switch(step)
	{
		...
		case 4:
			...
			sprintf((char*)tmp, "option value=\"%d\"", /*web_cfg.dtu.baud*/9600);
			web_body_add_option_str(web_buff, tmp, " selected=\"selected\"");
  			ret = web_data_send(s, web_buff, strlen(web_buff));
			return ret;
		case 5:
...
const char client_text1[]  = "<tr>\r\n\
    <td width=\"22%\" rowspan=\"2\"><div align=\"center\"><span class=\"STYLE16\">服务器</span></div></td>\r\n\
    <td height=\"26\"><div align=\"right\"><span class=\"STYLE16\">IP:</span></div></td>\r\n\
    <td><div align=\"left\"><span class=\"STYLE16\">\r\n\
      <input name=\"server_addr1\" type=\"text\" align=\"left\" id=\"server_addr1\" width=\"200\" maxlength=\"64\"/>\r\n\
    </span></div></td>\r\n\
  </tr>\r\n\
  <tr>\r\n\
    <td width=\"14%\" height=\"18\"><div align=\"right\"><span class=\"STYLE16\">PORT:</span></div></td>\r\n\
    <td width=\"64%\"><div align=\"left\"><span class=\"STYLE16\">\r\n\
      <input name=\"server_port1\" type=\"text\" align=\"left\" id=\"server_port1\" width=\"200\" maxlength=\"5\"/>\r\n\
    </span></div></td>\r\n\
  </tr>\r\n\
  <tr>\r\n\
...
const char client_text2[]  = "<tr>\r\n\
    <td rowspan=\"2\"><div align=\"center\">登陆</div></td>\r\n\
    <td height=\"26\"><div align=\"right\"><span class=\"STYLE16\">用户名:</span></div></td>\r\n\
    <td><div align=\"left\"><span class=\"STYLE16\">\r\n\
        <input name=\"usr_name\" type=\"text\" align=\"left\" id=\"usr_name\" width=\"200\" maxlength=\"64\"/>\r\n\
    </span></div></td>\r\n\
  </tr>\r\n\
  <tr>\r\n\
    <td height=\"18\"><div align=\"right\"><span class=\"STYLE16\">密码:</span></div></td>\r\n\
    <td><div align=\"left\"><span class=\"STYLE16\">\r\n\
        <input name=\"password\" type=\"tex\" align=\"left\" id=\"password\" width=\"200\" maxlength=\"5\"/>\r\n\
    </span></div></td>\r\n\
  </tr>\r\n";

const char client_text3[]  = "<tr>\r\n\
    <td rowspan=\"2\"><div align=\"center\">QOS</div></td>\r\n\
    <td height=\"26\"><div align=\"right\"><span class=\"STYLE16\">SUB:</span></div></td>\r\n\
    <td><div align=\"left\"><span class=\"STYLE16\">\r\n\
        <input name=\"qos_sub\" type=\"text\" align=\"left\" id=\"qos_sub\" width=\"200\" maxlength=\"64\"/>\r\n\
    </span></div></td>\r\n\
  </tr>\r\n\
  <tr>\r\n\
    <td height=\"18\"><div align=\"right\"><span class=\"STYLE16\">PUB:</span></div></td>\r\n\
    <td><div align=\"left\"><span class=\"STYLE16\">\r\n\
        <input name=\"qos_pub\" type=\"tex\" align=\"left\" id=\"qos_pub\" width=\"200\" maxlength=\"5\"/>\r\n\
    </span></div></td>\r\n\
  </tr>\r\n";

const char client_text4[]  = "<tr>\r\n\
    <td rowspan=\"2\"><div align=\"center\">TOPIC</div></td>\r\n\
    <td height=\"26\"><div align=\"right\"><span class=\"STYLE16\">SUB:</span></div></td>\r\n\
    <td><div align=\"left\"><span class=\"STYLE16\">\r\n\
        <input name=\"topic_sub\" type=\"text\" align=\"left\" id=\"topic_sub\" width=\"200\" maxlength=\"64\"/>\r\n\
    </span></div></td>\r\n\
  </tr>\r\n\
  <tr>\r\n\
    <td height=\"18\"><div align=\"right\"><span class=\"STYLE16\">PUB:</span></div></td>\r\n\
    <td><div align=\"left\"><span class=\"STYLE16\">\r\n\
        <input name=\"topic_pub\" type=\"tex\" align=\"left\" id=\"topic_pub\" width=\"200\" maxlength=\"5\"/>\r\n\
    </span></div></td>\r\n\
  </tr>\r\n";

/*
const char client_text1[]  = "<tr>\r\n\
    <td width=\"32%\" height=\"32\"><div align=\"right\"><span class=\"STYLE16\">服务器1 IP:</span></div></td>\r\n\
    <td width=\"68%\"><span class=\"STYLE16\">\r\n\
      <label>\r\n\
      <div align=\"left\">\r\n\
        <input name=\"server_addr1\" type=\"text\" id=\"server_addr1\" width=\"100\" maxlength=\"64\"/>\r\n\
        PORT:\r\n\
        <input name=\"server_port1\" type=\"text\" id=\"server_port1\" width=\"50\" maxlength=\"5\"/>\r\n\
      </div>\r\n\
      </label></span></td>\r\n\
  </tr>\r\n";
  */

const char server_addr_text[]  = "<input name=\"server_addr";
const char server_port_text[]  = "<input name=\"server_port";

const char usr_name_text[]  = "<input name=\"usr_name";
const char password_text[]  = "<input name=\"password";

const char qos_sub_text[]  = "<input name=\"qos_sub";
const char qos_pub_text[]  = "<input name=\"qos_pub";

const char topic_sub_text[]  = "<input name=\"topic_sub";
const char topic_pub_text[]  = "<input name=\"topic_pub";
...
int tcp_client_page_assemble(UCHAR s, CHAR _sub_step)
{
	...
	switch(step)
	{
		...
		case 3:
			...
			//str_replace(web_buff, "服务器1", "服务器1");
			//str_replace(web_buff, "server_addr1", "server_addr1");
			//str_replace(web_buff, "server_port1", "server_port1");

			num = 0;
			if(web_cfg.remote[num].server_name[num])
			{
				web_cfg.remote[num].server_name[64] = 0;
				sprintf((char*)tmp, " value=\"%s\" ", web_cfg.remote[num].server_name);
			}
			else
			{
				if(web_cfg.remote[num].server_ip)
				{
					sprintf(tmp, " value=\"%u.%u.%u.%u\" ", ip4_addr1(&web_cfg.remote[num].server_ip), 
					ip4_addr2(&web_cfg.remote[num].server_ip), 
					ip4_addr3(&web_cfg.remote[num].server_ip),  
					ip4_addr4(&web_cfg.remote[num].server_ip));
				}
			}
			...
			sprintf(tmp, " value=\"%u\" ", web_cfg.remote[0].port);
		...
		case 4:
			step = 5;
			assert(SIZEOF_STR(client_text2) < WEB_BUFF_MAX_VALID);
			memcpy(web_buff, client_text2, SIZEOF_STR(client_text2));
			web_buff[SIZEOF_STR(client_text2)] = 0;

			num = 0;
			
			sprintf(tmp, " value=\"%s\" ", web_cfg.remote[num].usr_name);

			web_body_add_input_str(web_buff, 
				usr_name_text, 
				tmp);

			sprintf(tmp, " value=\"%s\" ", web_cfg.remote[num].password);
		...
		case 5:
			step = 6;
			assert(SIZEOF_STR(client_text3) < WEB_BUFF_MAX_VALID);
			memcpy(web_buff, client_text3, SIZEOF_STR(client_text3));
			web_buff[SIZEOF_STR(client_text3)] = 0;

			num = 0;
			
			sprintf(tmp, " value=\"%d\" ", web_cfg.remote[num].qos_sub);

			web_body_add_input_str(web_buff, 
				qos_sub_text, 
				tmp);

			sprintf(tmp, " value=\"%d\" ", web_cfg.remote[num].qos_pub);
		...
		case 6:
			step = 12;
			assert(SIZEOF_STR(client_text4) < WEB_BUFF_MAX_VALID);
			memcpy(web_buff, client_text4, SIZEOF_STR(client_text4));
			web_buff[SIZEOF_STR(client_text4)] = 0;

			num = 0;
			
			sprintf(tmp, " value=\"%s\" ", iot_work.self_sn.sn/*web_cfg.remote[num].topic_sub*/);

			web_body_add_input_str(web_buff, 
				topic_sub_text, 
				tmp);
		
			iot_work.self_sn.sn[31] += 1;
			sprintf(tmp, " value=\"%s\" ", iot_work.self_sn.sn/* web_cfg.remote[num].topic_pub*/);
			iot_work.self_sn.sn[31] -= 1;	
...
//	int auth_mode;
	...
	//web_cfg.sta.mode = (i == 1)?1:0;

	i = get_post_tag_value("ap_channel", buff, 0);
	if(i > 0 && i < 14)
		i = i;
	else
		i = 1;
	web_cfg.ap.channel = i;

	i = get_post_tag_value("ap_ssid", buff, ssid);
	if(i == 0)
	{
		if(strlen(ssid) < 32)
			strcpy(web_cfg.ap.essid, ssid);
	}
	i = get_post_tag_value("ap_key", buff, key);
	if(i == 0)
	{
		if(strlen(key) < 32)
			strcpy(web_cfg.ap.key, key);
	}else{
		memset(web_cfg.ap.key, 0, 14);
	}
	
	i = get_post_tag_value("ap_security", buff, 0);
	web_cfg.ap.auth = i;

	i = get_post_tag_value("sta_ssid", buff, ssid);
	if(i == 0)
	{
		if(strlen(ssid) < 32)
			strcpy(web_cfg.sta.essid, ssid);
	}
	i = get_post_tag_value("sta_key", buff, key);
	if(i == 0)
	{
		if(strlen(key) < 32)
			strcpy(web_cfg.sta.key, key);
	}else{
		memset(web_cfg.sta.key, 0, 14);
	}
...
int handle_eth_ip_addr_post(int s, char *buff)
{
	int i;
	char tmp[64];
	p_dbg_enter;
	...
	i = get_post_tag_value("ip_ipaddr", buff, tmp);
	if(i == 0){
		web_cfg.eth_ip.ip = inet_addr(tmp);
	}

	i = get_post_tag_value("ip_gw", buff, tmp);
	if(i == 0){
		web_cfg.eth_ip.gw = inet_addr(tmp);
	}

	i = get_post_tag_value("ip_mask", buff, tmp);
	if(i == 0){
		web_cfg.eth_ip.msk = inet_addr(tmp);
	}

	i = get_post_tag_value("ip_dns", buff, tmp);
	if(i == 0){
		web_cfg.eth_ip.dns = inet_addr(tmp);
	}

	i = get_post_tag_value("ip_auto", buff, 0);
	if(i != 1)
		web_cfg.eth_ip.auto_get = 0;
	else
		web_cfg.eth_ip.auto_get = 1;
...
int handle_audio_ctl_post(int s, char *buff)
{
	int i;
	p_dbg_enter;

	i = get_post_tag_value("audio_fre", buff, 0);
	if(i >= 0){
		web_cfg.audio.audio_fre = i;
	}

	i = get_post_tag_value("audio_qua", buff, 0);
	if(i >= 0){
		web_cfg.audio.audio_qua = i;
	}
...
void get_remote_server_params(char *buff, int num)
{
	...
	if(num >= REMOTE_SERVER_NUM)
		return;

	sprintf(server_addr_str, "server_addr%d", num + 1);
	sprintf(server_port_str, "server_port%d", num + 1);
	i = get_post_tag_value(server_addr_str, buff, tmp);
	if(i == 0)
	{
		if(tmp[0] >= '0' && tmp[0] <= '9')
		{
			web_cfg.remote[num].server_ip = inet_addr(tmp);
			memset(web_cfg.remote[num].server_name, 0, 64);
		}
		else
		{
			web_cfg.remote[num].server_ip = 0;
			strcpy(web_cfg.remote[num].server_name, tmp);
		}
	}else{
		web_cfg.remote[num].server_ip = 0;
	}

	i = get_post_tag_value(server_port_str, buff, 0);
	if(i < 0)
		web_cfg.remote[num].port = 0;
	else
		web_cfg.remote[num].port = i;
...
int handle_tcp_client_post(int s, char *buff)
{
	...
	i = get_post_tag_value("server_addr1", buff, tmp);
	if(i == 0)
	{
		if(tmp[0] >= '0' && tmp[0] <= '9')
		{
			web_cfg.remote[0].server_ip = inet_addr(tmp);
			memset(web_cfg.remote[0].server_name, 0, 64);
		}
		else
		{
			web_cfg.remote[0].server_ip = 0;
			strcpy(web_cfg.remote[0].server_name, tmp);
		}
	}else{
		web_cfg.remote[0].server_ip = 0;
	}

	i = get_post_tag_value("server_port1", buff, 0);
	if(i < 0)
		web_cfg.remote[0].port = 0;
	else
		web_cfg.remote[0].port = i;

	i = get_post_tag_value("qos_sub", buff, 0);
	if(i < 0)
		web_cfg.remote[0].qos_sub = 0;
	else
		web_cfg.remote[0].qos_sub = i;

	i = get_post_tag_value("qos_pub", buff, 0);
	if(i < 0)
		web_cfg.remote[0].qos_pub = 0;
	else
		web_cfg.remote[0].qos_pub = i;

	i = get_post_tag_value("usr_name", buff, tmp);
	if(i == 0)
	{
		strcpy(web_cfg.remote[0].usr_name, tmp);
	}else{
		web_cfg.remote[0].usr_name[0] = 0;
	}
	
	i = get_post_tag_value("password", buff, tmp);
	if(i == 0)
	{
		strcpy(web_cfg.remote[0].password, tmp);
	}else{
		web_cfg.remote[0].password[0] = 0;
	}

	i = get_post_tag_value("topic_sub", buff, tmp);
	if(i == 0)
	{
		//strcpy(web_cfg.remote[0].topic_sub, tmp);
	}else{
		//web_cfg.remote[0].topic_sub[0] = 0;
	}

	i = get_post_tag_value("topic_pub", buff, tmp);
	if(i == 0)
	{
		//strcpy(web_cfg.remote[0].topic_pub, tmp);
	}else{
		//web_cfg.remote[0].topic_pub[0] = 0;
	}
	...
	switch(step)
	{
		...
		case 2:
			...
			sprintf(web_buff, "<tr bordercolor=\"#F0F0F0\">\r\n\
    			<td height=\"94\" colspan=\"2\"><div align=\"center\"><span class=\"STYLE18\"><p>欢迎使用秉火科技的产品</p></span></div>\r\n\
				<p>电话:88888888</p>\r\n\
				<p>QQ:88888888</p>\r\n\
				<p>更多产品请关注:www.bhlink.cn</p>\r\n\
      			<hr /></td>\r\n\
  				</tr>\r\n");
  			ret = web_data_send(s, web_buff, strlen(web_buff));
			return ret;
...
void test_jpeg_send(UCHAR s, CHAR _step)
{
	#if 0
	static uint8_t step = 0;
	int img_size;
	...
	web_cfg.cur_routing = test_jpeg_send;
	p_dbg("logo step:%d", step);

	img_size = /*get_jpeg_size()*/8192;
	
	if(step == 0)
	{
		sprintf(web_buff, "%s%d\r\n\r\n", http_jpeg_head, img_size);
		web_data_send(s, web_buff, strlen(web_buff));
		step = 1;
	}else if(step == 1){
		web_data_send(s, (const char*)camera_cfg.dma_buff[0], img_size);	
		web_cfg.cur_routing = 0;
		step = 0;
		start_capture_img();
	}
...
void handle_web_get(UCHAR s)
{
	if(web_cfg.get_name[0] == 0 || strcmp(web_cfg.get_name, str_index_page) == 0){
		...
		sub_page_send(s, 0);
	}else if(strcmp(web_cfg.get_name, str_eth_ip_addr_page) == 0){
		web_cfg.assemble_routing = ip_eth_addr_page_assemble;
		sub_page_send(s, 0);
		...
		sub_page_send(s, 0);
//	}else if(strcmp(web_cfg.get_name, str_audio_ctl_page) == 0){
//		web_cfg.assemble_routing = audio_page_assemble;
//		sub_page_send(s, 0);
	}else if(strcmp(web_cfg.get_name, str_pic_ctl_page) == 0){
		web_cfg.assemble_routing = pic_page_assemble;
		sub_page_send(s, 0);
		...
		sub_page_send(s, 0);
	}else if(strcmp(web_cfg.get_name, str_test_pic) == 0){
		test_jpeg_send(s, 0);
...
void handle_web_post(int s)
{
	if(strcmp(web_cfg.get_name, str_wifi_link_page) == 0){
		...
		handle_eth_ip_addr_post(s, web_buff);
	}else if(strcmp(web_cfg.get_name, str_tcp_server_page) == 0){
		handle_tcp_server_post(s, web_buff);
	}else if(strcmp(web_cfg.get_name, str_tcp_client_page) == 0){
		handle_tcp_client_post(s, web_buff);
//	}else if(strcmp(web_cfg.get_name, str_audio_ctl_page) == 0){
//		handle_audio_ctl_post(s, web_buff);
	}else if(strcmp(web_cfg.get_name, str_reset_page) == 0){
...
void web_server_thread(void *arg)
{
	...
	while(1)
	{
		...
		if(ret == -1)
		{
			...
			add_select_array(ret);
	}
}
unsigned char debug_s = 0;
...
void default_web_cfg(WEB_CFG *cfg)
{
	...
	cfg->ap.channel = 6;
	cfg->ap.mode= MODE_AP;
	strcpy(cfg->ap.essid, "OV2640");
	strcpy(cfg->ap.key, "12345678");

	cfg->sta.channel = 6;
	cfg->sta.mode= MODE_STATION;
//	strcpy(cfg->sta.essid, "A304");
//	strcpy(cfg->sta.key, "1234567890123");
	strcpy(cfg->sta.essid, "A305");
	strcpy(cfg->sta.key, "wildfire");

	cfg->ip.ip = inet_addr("192.168.0.8");
	cfg->ip.gw = inet_addr("192.168.0.1");
	cfg->ip.msk= inet_addr("255.255.255.0");
	cfg->ip.dns = inet_addr("8.8.8.8");
	cfg->ip.auto_get = 1;

	cfg->eth_ip.ip = inet_addr("192.168.1.8");
	cfg->eth_ip.gw = inet_addr("192.168.1.1");
	cfg->eth_ip.msk= inet_addr("255.255.255.0");
	cfg->eth_ip.dns = inet_addr("8.8.8.8");
	cfg->eth_ip.auto_get = 0;
...
//	cfg->audio.audio_qua = SPEEX_DEFAULT_QUALITY;
	...
	//cfg->remote[0].server_ip = inet_addr("120.25.210.138");
	//cfg->remote[0].port = 1883;
	cfg->remote[0].server_ip = inet_addr("120.25.3.35");
	cfg->remote[0].port = 1883;
...
//	uint32_t Address;
	...
	ret =w25q128_erase(WEB_CFG_ADDR, SAVE_WEB_CFG_SIZE(cfg));
	if(ret)
		return -1;
	ret = w25q128_write(WEB_CFG_ADDR, SAVE_WEB_CFG_SIZE(cfg), (void*)cfg);
	if(ret)
		return -1;
...
int load_web_cfg(WEB_CFG *cfg)
{
	...
	ret = w25q128_read(WEB_CFG_ADDR, SAVE_WEB_CFG_SIZE(cfg), (void*)cfg);	
	if(ret != 0 || !verify_web_cfg(cfg) /*|| !BUTTON3_STAT*/)
	{
		default_web_cfg(cfg);
		save_web_cfg(cfg);
	}
