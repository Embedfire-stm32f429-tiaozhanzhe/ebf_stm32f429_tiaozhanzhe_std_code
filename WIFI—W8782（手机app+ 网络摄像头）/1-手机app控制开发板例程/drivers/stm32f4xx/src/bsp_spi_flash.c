 ...
 */
static int read_sr(struct w25q *flash)
{
	ssize_t retval;
	u8 code = OPCODE_RDSR;
	u8 val;
 ...
 */
 /*
static int write_sr(struct w25q *flash, u8 val)
{
 ...
 * Returns negative if error occurred.
 */
static int write_enable(struct w25q *flash)
{
	u8	code = OPCODE_WREN;
 ...
 */
/*
static int write_disable(struct w25q *flash)
{
	u8	code = OPCODE_WRDI;
 ...
 */
 #if 0
static inline int set_4byte(struct w25q *flash, u32 jedec_id, int enable)
{
 ...
 */
static int wait_till_ready(struct w25q *flash)
{
 ...
 */
static int erase_chip(struct w25q *flash)
{
	p_dbg("%s: %s %lldKiB\n",
	      "", __FUNCTION__,
...
static void w25q_addr2cmd(struct w25q *flash, unsigned int addr, u8 *cmd)
{
	/* opcode is in cmd[0] */
...
static int w25q_cmdsz(struct w25q *flash)
{
 ...
 * Returns 0 if successful, non-zero otherwise.
 */
static int erase_sector(struct w25q *flash, u32 offset)
{
	...
	w25q_addr2cmd(flash, offset, flash->command);

	spi_write((char*)flash->command, w25q_cmdsz(flash));
...
int w25q128_read(u32 from, size_t len, u_char *buf)
{
	...
	w25q_addr2cmd(flash, from, flash->command);

	spi_write_then_read((char*)flash->command, w25q_cmdsz(flash) + FAST_READ_DUMMY_BYTE, (char*)buf,len);
...
int cmp_flash_data(int addr, int size, const u_char *buff)
{
	...
	w25q_addr2cmd(flash, addr, check_cmd);
	spi_write_then_read((char*)check_cmd, 
		w25q_cmdsz(flash) + FAST_READ_DUMMY_BYTE, 
		(char*)check_buff,
		size);
...
//	FLASH_WP_DISABLE;;
	...
	w25q_addr2cmd(flash, to, flash->command);
	...
	if (page_offset + len <= flash->page_size) {
		...
		spi_write_then_write((char*)flash->command, w25q_cmdsz(flash), (char*)buf, len);
/*
		...
		spi_write_then_write((char*)flash->command, w25q_cmdsz(flash), (char*)buf, page_size);
/*
...
*/
		...
		for (i = page_size; i < len; i += page_size) {
			...
			w25q_addr2cmd(flash, to + i, flash->command);

			wait_till_ready(flash);

			write_enable(flash);

			spi_write_then_write((char*)flash->command, w25q_cmdsz(flash), (char*)buf + i, page_size);
/*
...
int read_firmware(u32 offset, u32 len, u8 * pbuf)
{
	...
	return w25q128_read(FIRMWARE_BASE_ADDR + offset, len, pbuf);
}
...
int test_write_firmware_to_spi_flash()
{
	...
	ret = w25q128_erase(FIRMWARE_BASE_ADDR, fm_size);
	if(ret){
		p_err("flash erase err,ret = %d",ret);
		return -1;
	}
	
	ret = w25q128_write(FIRMWARE_BASE_ADDR, fm_size, fm_88w8287);
	if(ret){
...
int  w25q128_init(void)
{
	...
	memset(&w25_flash, 0, sizeof(struct w25q));
	w25_flash.addr_width = 3;
	w25_flash.sector_size = 4*1024;
	w25_flash.page_size = 256;
	w25_flash.enable = TRUE;
	w25_flash.size = FLASH_ROOM_SIZE;	//1mBYTE
	w25_flash.lock = mutex_init(__FUNCTION__);
...
//{
//	SPI_I2S_ClearITPendingBit(SPI5, SPI_I2S_IT_ERR);
//}
