...
static void DMA_I2S3configuration(uint32_t *BufferSRC, uint32_t BufferSize)
{
	...
	DMA_ClearFlag(DMA1_Stream6, DMA_FLAG_TCIF5 | DMA_FLAG_TEIF5 | DMA_FLAG_HTIF5 | DMA_FLAG_DMEIF5 | DMA_FLAG_HTIF5);
...
{	
	...
	if(DMA_GetITStatus(DMA1_Stream6, DMA_IT_TCIF5))
	{
	...
	if(DMA_GetITStatus(DMA1_Stream6, DMA_IT_HTIF5))
	{
		DMA_ClearITPendingBit(DMA1_Stream6, DMA_IT_HTIF5);
		//if(audio_cfg.pcm->sync)
		{
			dec_output.play_circle_cnt++;
	...
	if(DMA_GetITStatus(DMA1_Stream6, DMA_IT_TEIF5))
	{
		DMA_ClearFlag(DMA1_Stream6,  DMA_IT_TEIF5);
		p_err("DMA_IT_TEIF5");
	}
...
void DMA1_Stream6_IRQHandler()  //dac1
{	
	enter_interrupt();
...
//	struct DATA_CHUNK *chunk;
	...
	while(1)
	{
		...
		if(audio_cfg.recording)
		{
			...
			for(i = 0; i < frames; i++)
			{
				ret = iot_speex_encode((uint16_t *)src_data + i*SPEEX_ENC_FRAME_SIZE, SPEEX_ENC_FRAME_SIZE, out_data,  src_len);
				//play_speex_stream(out_data, ret); 

				if(ret != SPEEX_DEC_FRAME_SIZE)
					p_err_fun;

				out_data += SPEEX_DEC_FRAME_SIZE;
			}
