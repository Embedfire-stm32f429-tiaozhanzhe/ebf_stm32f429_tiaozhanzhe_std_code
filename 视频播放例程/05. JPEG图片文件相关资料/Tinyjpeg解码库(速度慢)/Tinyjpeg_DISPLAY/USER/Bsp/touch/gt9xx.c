...
/**
  ...
  * @retval 无
  */
static uint16_t pre_x[GTP_MAX_TOUCH] ={ LCD_PIXEL_WIDTH*2,LCD_PIXEL_WIDTH*2,LCD_PIXEL_WIDTH*2,LCD_PIXEL_WIDTH*2,LCD_PIXEL_WIDTH*2};
static uint16_t pre_y[GTP_MAX_TOUCH] ={ LCD_PIXEL_HEIGHT*2,LCD_PIXEL_HEIGHT*2,LCD_PIXEL_HEIGHT*2,LCD_PIXEL_HEIGHT*2,LCD_PIXEL_HEIGHT*2};
...
//    LCD_DrawLine(x, y, w, LCD_DIR_HORIZONTAL);
//    LCD_DrawFullCircle(x,y,10);

    if(pre_x[id]>LCD_PIXEL_WIDTH||pre_y[id]>LCD_PIXEL_WIDTH)
    {
#if 1
    	...
    	LCD_DrawUniLine(pre_x[id],pre_y[id],x,y);
    
    #if 0
        sprintf(buf,"(%d,%d)",x,y);

    LCD_ClearLine(LINE(1));
        LCD_DisplayStringLine(LINE(1),(uint8_t *)buf);
...
{
	  pre_x[id] = LCD_PIXEL_WIDTH*2;
	  pre_y[id] = LCD_PIXEL_HEIGHT*2;
