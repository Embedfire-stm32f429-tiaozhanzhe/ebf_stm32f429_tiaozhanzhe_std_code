...
{ 
  ...
  LTDC_InitStruct.LTDC_HorizontalSync =HSYNC;// 9;
  /* Configure vertical synchronization height */
  LTDC_InitStruct.LTDC_VerticalSync = VSYNC;//1;
  /* Configure accumulated horizontal back porch */
  LTDC_InitStruct.LTDC_AccumulatedHBP =HBP;// 29; 
  /* Configure accumulated vertical back porch */
  LTDC_InitStruct.LTDC_AccumulatedVBP = VBP;//3;  
  /* Configure accumulated active width */  
  LTDC_InitStruct.LTDC_AccumulatedActiveW = LCD_PIXEL_WIDTH+HSYNC+HBP-1;//269;
  /* Configure accumulated active height */
  LTDC_InitStruct.LTDC_AccumulatedActiveH = LCD_PIXEL_HEIGHT+VSYNC+VBP-1;//323;
  /* Configure total width */
  LTDC_InitStruct.LTDC_TotalWidth = 1075;//LTDC_InitStruct.LTDC_AccumulatedActiveW +HSYNC-1;//279; 
  /* Configure total height */
  LTDC_InitStruct.LTDC_TotalHeigh = 534;//LTDC_InitStruct.LTDC_AccumulatedActiveH +VSYNC-1;//327;
...
void LCD_LayerInit(void)
{
  ...
  LTDC_Layer_InitStruct.LTDC_HorizontalStart = 66;//HSYNC+HBP;//30;
  LTDC_Layer_InitStruct.LTDC_HorizontalStop = (LCD_PIXEL_WIDTH + LTDC_Layer_InitStruct.LTDC_HorizontalStart - 1); 
  LTDC_Layer_InitStruct.LTDC_VerticalStart =33;//VSYNC+VBP; //4;
  LTDC_Layer_InitStruct.LTDC_VerticalStop = (LCD_PIXEL_HEIGHT + LTDC_Layer_InitStruct.LTDC_VerticalStart - 1);
...
void LCD_Clear(uint32_t Color)
{
  ...
  uint32_t index = 0;
    ...
    for (index = 0x00; index < LCD_PIXCELS; index++)
  {
    *(__IO uint16_t*)(CurrentFrameBuffer + (index)) = (0x00FFFF & Color);           //GB
    *(__IO uint8_t*)(CurrentFrameBuffer + (++index+2)) = (0xFF0000 & Color) >> 16;  //R
  } 
  ...
  {                                                      
    ...
    DMA2D_InitStruct.DMA2D_NumberOfLine = LCD_PIXEL_WIDTH;
    DMA2D_InitStruct.DMA2D_PixelPerLine = LCD_PIXEL_HEIGHT;
  }
...
void LCD_DrawColorRect(uint16_t Xpos, uint16_t Ypos, uint16_t Width, uint16_t Height,uint8_t *color)
{
  uint32_t Address,index;
  uint32_t currentline = 0, linenumber = 0;

  Address = CurrentFrameBuffer + 2*(LCD_PIXEL_WIDTH*Ypos + Xpos);

  for(index = 0; index < Width*Height; index++)
  {
   *(__IO uint16_t*) (Address) = *(__IO uint16_t *)color;
   /*jump on next byte */
   color+=2;
   Address+=2;
   currentline++;
   ...
   if((currentline) == Width)
   {
     ...
     {
       linenumber++;
       Address +=((LCD_PIXEL_WIDTH)*2-Width*2);
       currentline = 0;
     }
   }
  }
