...
void LCD_Clear(uint32_t Color)
{
  ...
  for (index = 0x00; index < LCD_PIXEL_WIDTH*LCD_PIXEL_HEIGHT*3; index++)
  {
...
void LCD_WriteBMP(uint32_t BmpAddress)
{
 ...
 Address += width*(height-1)*(bit_pixel/8);
  ...
  for(index = 0; index < size*8/bit_pixel; index++)
  {
    *(__IO uint16_t*) (Address) = *(__IO uint16_t *)BmpAddress;
    /*jump on next byte */
    BmpAddress+=2;
    Address+=2;
    if(bit_pixel/8==3)
    ...
    if(currentline == width)
    {
      ...
      {
        linenumber++;
        Address -=((LCD_PIXEL_WIDTH+width)*(bit_pixel/8));
        currentline = 0;
      }
    }
  }
