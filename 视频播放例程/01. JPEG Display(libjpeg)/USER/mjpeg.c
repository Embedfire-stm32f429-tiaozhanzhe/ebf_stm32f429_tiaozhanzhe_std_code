...
read_JPEG_file (char * filename)
{
  ...
  /* Open a JPEG file */
  fres = f_open(&jpgFile,filename,FA_READ|FA_OPEN_EXISTING);		 
  if(fres != FR_OK)
  {
  ...
  fres = f_read(&jpgFile,jpegbuf,jpgFile.fsize,&fnum);		 
  if(fres != FR_OK)
  {
...
//    put_scanline_someplace(buffer[0], row_stride);
    LCD_DrawColorRect(0,cinfo.output_scanline-1,cinfo.output_width,1,buffer[0]);
  }
