...
void LCD_Clear(uint32_t Color)
{
  ...
  for (index = 0x00; index < LCD_PIXCELS*3; index++)
  {
...
static void DMA_Config_MEM2MEM(uint8_t * aDMA_PeripheralAddr,uint32_t aDMA_MemoryAddr,uint32_t BUFFER_SIZE)
{
  ...
  DMA_InitStructure.DMA_PeripheralBaseAddr = (uint32_t)aDMA_PeripheralAddr;
  DMA_InitStructure.DMA_Memory0BaseAddr = aDMA_MemoryAddr;
...
void LCD_DrawColorRect(uint16_t Xpos, uint16_t Ypos, uint16_t Width, uint16_t Height,uint8_t *color)
{
  ...
  Address = CurrentFrameBuffer + 3*LCD_PIXEL_WIDTH*Ypos;  
  for(index = 0; index < Width; index++)
  {
//    *(__IO uint32_t*) (Address) = ((*color)<<16 | (*(color+1)<< 8) | (*(color+2)));  
    
    *(__IO uint16_t*) (Address) =  (*(color+1)<< 8) | (*(color+2));     
    *(__IO uint8_t*) (Address+2) =*color; 
    
//    *(__IO uint8_t*) Address     =*(color+2);      
//    *(__IO uint8_t*) (Address+1) =*(color+1);      
//    *(__IO uint8_t*) (Address+2) =*color;     
    /*jump on next byte */
    color+=3;
    Address+=3;
  ...
  uint32_t Address;
  Address = CurrentFrameBuffer + 3*LCD_PIXEL_WIDTH*Ypos;  
  DMA_Config_MEM2MEM(color,Address,3*Width/2);
  while(DMA_GetFlagStatus(DMA_STREAM,DMA_FLAG_TCIF)==RESET);
#endif
}
